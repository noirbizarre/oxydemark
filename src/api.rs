//! Pure-Rust public API: parsing, rendering, slugging, and summary extraction.
//!
//! This module is completely independent of PyO3. The optional Python bindings
//! (`crate::python`, behind the `python` cargo feature) are thin wrappers over
//! these functions.
...
use crate::ast::{self, AstNode, ParseResult};
...
use crate::html_render::render_ast_to_html;
use crate::slug;
...
/// Returns an `AstNode` tree that can be inspected and modified.
pub fn parse(markdown: &str) -> AstNode {
    with_parser(|parser| {
        ...
        ast::arena_to_ast_node(&arena, document_ref, markdown)
    })
}
...
///
/// This is the second half of the pipeline, used after plugins
/// have had a chance to modify the AST.
pub fn render_ast(node: &AstNode) -> String {
...
///
/// Returns [`OxydeError::Render`] if the renderer fails.
pub fn markdown_to_html(markdown: &str) -> Result<String, OxydeError> {
    with_parser(|parser| {
        let mut reader = text::BasicReader::new(markdown);
        let (mut arena, document_ref) = parser.parse(&mut reader);
        assign_heading_anchors(&mut arena, document_ref, markdown);
        ...
        with_renderer(|renderer| {
            ...
            renderer
                ...
                .map_err(|e| OxydeError::Render(format!("{e}")))?;
            Ok(output)
        })
    })
}
...
mod tests {
    ...
    use crate::extensions;
    use crate::html_render::{collect_text, html_escape};
    ...
    #[test]
    fn parse_link() {
        let ast = parse("[click](https://example.com)");
        let para = &ast.children[0];
    ...
    fn parse_link_with_title() {
        let ast = parse(r#"[click](https://example.com "a title")"#);
        let para = &ast.children[0];
        let link = para.children.iter().find(|c| c.kind == "link").unwrap();
        assert_eq!(
            link.attributes.get("href").map(|v| v.as_str()),
    ...
    #[test]
    fn parse_image() {
        let ast = parse("![alt text](image.png)");
        let para = &ast.children[0];
