...
This namespace exists to demonstrate the [`Plugin`][oxydemark.Plugin] protocol
...
| --- | --- | --- |
| [`AdmonitionPlugin`][oxydemark.contrib.AdmonitionPlugin] | `preprocess` + `transform` | text normalisation, then AST enrichment |
| [`ShortcodePlugin`][oxydemark.contrib.ShortcodePlugin] | `transform` | replacing text nodes with `raw_html` |
| [`MentionPlugin`][oxydemark.contrib.MentionPlugin] | `transform` | splitting text into structural nodes |
| [`LazyImagesPlugin`][oxydemark.contrib.LazyImagesPlugin] | `postprocess` | pure HTML-string rewriting |

See the [plugin authoring guide](../plugins.md) for the full picture.
